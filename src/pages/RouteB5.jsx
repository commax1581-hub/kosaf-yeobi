...
function genHTML(data,cats,imgs,extra,amt,adjs){
  ...
  const pg=(title,items,big)=>items.length?"<div style='page-break-before:always;padding:20px'><h1>"+title+"</h1>"+(big?items.map(it=>"<div class=il><b>"+it.label+"</b><br/><img src='"+it.img.base64+"' style='width:100%;max-height:600px;object-fit:contain'/></div>").join(""):"<div class=i2>"+items.map(it=>"<div><b>"+it.label+"</b><br/><img src='"+it.img.base64+"' style='width:100%;max-height:400px;object-fit:contain'/></div>").join("")+"</div>")+"</div>":"";
